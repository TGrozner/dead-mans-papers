...
test('starts with a bounded tutorial and opens the first dialogue', async ({ page }) => {
  ...
  const wakeChoices = page.locator('.dialogue-root .choice-button')
  const phoneChoice = wakeChoices.filter({ hasText: "Chercher le téléphone à l'aveugle" })
  const doseChoice = wakeChoices.filter({ hasText: 'Rester dans le noir' })
  const groundChoice = wakeChoices.filter({ hasText: 'Sentir le béton' })
  const standChoice = wakeChoices.filter({ hasText: 'Avaler la bile' })

  await expect(phoneChoice).toHaveAttribute('data-choice-personality', 'procedure')
  await expect(phoneChoice.locator('.choice-personality')).toHaveText('Procédure')
  await expect(doseChoice).toHaveAttribute('data-choice-personality', 'dose faim')
  await expect(doseChoice.locator('.choice-personality')).toHaveText(['La Dose', 'Corps'])
  await expect(groundChoice).toHaveAttribute('data-choice-personality', 'memoire_saline')
  await expect(groundChoice.locator('.choice-personality')).toHaveText('Mémoire')
  await expect(standChoice).toHaveAttribute('data-choice-personality', 'faim honte_publique')
  await expect(standChoice.locator('.choice-personality')).toHaveText(['Corps', 'Honte'])
  await page.keyboard.press('Tab')
